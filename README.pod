...
B<Args> C<$encode-me where Buf|Str|Blob, :$pad, :@alpha, Bool :$str>
...
Takes a C<Buf> and applies base64 encoding with the requested options. If passed a C<Str> it will be converted to a C<Buf> via C<.ords> first. If passed a C<Blob> it will be converted to a C<Buf> first.
